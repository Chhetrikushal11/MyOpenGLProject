...
private:
    ...
    RenderComponent make_obj_mesh(const char* filepath);  // CHANGED: Now returns RenderComponent
    ...
    void read_corner(
        ...
        std::vector<glm::vec3>& v,
        std::vector<glm::vec2>& vt,   // FIXED: Correct type
        std::vector<glm::vec3>& vn,   // FIXED: Correct type
        std::vector<float>& vertices
    );
};