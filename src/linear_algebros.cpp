...
mat4 create_matrix_transform(vec3 translation)
{
	...
	matrix.entries[5] =  1.0;
	matrix.entries[6] =  0.0;
	matrix.entries[7] =  0.0;

	matrix.entries[8] =  0.0;
	matrix.entries[9] =  0.0;
	matrix.entries[10] = 1.0;
...
{
	float result = v.entries[0] * u.entries[0] + v.entries[1] * u.entries[1] + v.entries[2] * u.entries[2];
		return result;
}
